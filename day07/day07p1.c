...
int can_contain(char *bag_color, char *color);
int can_contain(char *bag_color, char *color) {
	for(unsigned int i = 0; i < n_bags; i++) {
		if(strcmp(bags[i].color, bag_color) == 0) {
			for(unsigned int j = 0; j < bags[i].contents.n_colors; j++) {
				if(strcmp(bags[i].contents.colors[j], color) == 0) {
					return 1;
				} else if(can_contain(bags[i].contents.colors[j], color)) {
					return 1;
				}
			}
			break;
...
int main(int argc, const char **argv) {
	if(argc != 2) {
		fprintf(stderr, "Usage: day07p1 <input>\n");
		return EXIT_FAILURE;
	}
	...
	for(unsigned int i = 0; i < n_bags; i++) {
		if(can_contain(bags[i].color, "shiny gold")) {
			printf("%s can contain shiny gold\n", bags[i].color);
			result++;
		}
	}
